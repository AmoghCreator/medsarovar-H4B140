...
app.post('/sendAppointment', async (req, res) => {
  ...
  await Appointment.create({name:user.name, dateandtime:req.body.dateandtime})
...
bot.on('message', async (msg) => {
  ...
  switch (userState) {
    ...
    case 'awaiting_phone':
      ...
      bot.sendMessage(chatId, "Please provide the maximum amount you can spend on a doctor's visit");
      break;
    ...
    case 'awaiting_budget':
      ...
      await User.create({ chat_id: chatId, name: name, age: age, phone: phone, budget: budget });

      bot.sendMessage(chatId, `Thank you, ${name}. Your details have been recorded.`);
  ...
  if (!userContext[chatId] || userContext[chatId] === 'initial') {
    ...
    try {
      ...
      const diagnosisResponse = await openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [{"role": "user", "content": `Please provide a possible diagnosis for the following symptoms: ${userMessage}`}],
      });

      const specialtyResponse = await openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [{"role": "user", "content": `Which medical specialty is required for the following symptoms: ${userMessage}`}],
      });
      ...
      bot.sendMessage(chatId, 'Here are some doctors you can book an appointment with:', options);
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (userContext[chatId] && userContext[chatId].state === 'selecting_doctor') {
    ...
    bot.sendMessage(chatId, `You selected  ${selectedDoctor.doctorName}. Booking an appointment...`);

    // Handle the booking process and make the API call
    try {
      const response = await axios.get('https://hack4bengal-427819.el.r.appspot.com/call', {
      ...
      if (response.status === 200) {
        ...
        bot.sendMessage(chatId, `Failed to book appointment with  ${selectedDoctor.doctorName}. Please try again.`);
      }
    } catch (error) {
      console.error('Error booking appointment:', error);
      bot.sendMessage(chatId, `Error booking appointment with  ${selectedDoctor.doctorName}. Please try again later.`);
    }
  }
});
